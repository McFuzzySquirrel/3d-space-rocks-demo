...
- **Every major framework** gets a specialist (e.g., `babylonjs-specialist`, `react-specialist`, `django-specialist`).
- **Every distinct functional domain** gets an agent (e.g., `gameplay-engineer`, `auth-engineer`, `payments-specialist`).
- **Cross-cutting concerns** get agents when they're substantial (e.g., `audio-engineer`, `vfx-artist`, `devops-engineer`).
...
- Agent names must be lowercase with hyphens, matching the filename (e.g., `gameplay-engineer.md` → `name: gameplay-engineer`).
- Skill names must be lowercase with hyphens, matching the directory name (e.g., `create-game-entity/SKILL.md` → `name: create-game-entity`).
...
- **build-prd** skill — If no PRD exists yet, recommend using the `build-prd` skill first to create one, then run this process against the resulting PRD.
