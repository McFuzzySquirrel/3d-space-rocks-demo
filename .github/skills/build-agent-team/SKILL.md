...
**Naming conventions:**
- Agent names must be **lowercase with hyphens**: `gameplay-engineer`, `ui-hud-developer`.
...
|---|---|---|
| **Scaffold [entity type]** | When multiple similar entities/components will be created | `create-game-entity`, `create-api-endpoint`, `create-react-component` |
| **Set up [subsystem]** | When a complex subsystem requires multi-step initialization | `setup-babylonjs-scene`, `setup-database`, `setup-auth` |
| **Create [effect/asset]** | When a category of assets will be produced repeatedly | `create-particle-effect`, `create-migration`, `create-test-suite` |
...
- Lowercase with hyphens: `create-game-entity`, `setup-database`.
...
The 3D Space Rocks project in this repository demonstrates a complete team built from a game PRD:

**Agents** (`.github/agents/`):
- `project-architect` — Vite + TypeScript scaffolding, dependencies
- `babylonjs-specialist` — 3D scene, camera, materials, meshes
- `gameplay-engineer` — State machine, player, asteroids, waves, scoring
- `physics-engineer` — cannon-es, collisions, physics tuning
- `ui-hud-developer` — Babylon.js GUI, HUD, menus, overlays
- `vfx-artist` — Particles, explosions, thruster effects, camera shake
- `audio-engineer` — Sound manager, SFX, spatial audio
- `pwa-specialist` — Service Worker, manifest, offline caching
- `qa-tester` — Vitest, unit/integration tests, test scenarios

**Skills** (`.github/skills/`):
- `build-prd` — Create PRDs from ideas/research
- `create-game-entity` — Scaffold TypeScript entity classes
- `setup-babylonjs-scene` — Initialize engine, scene, camera, lighting
- `create-particle-effect` — Particle system factory templates
- `implement-ui-screen` — Babylon.js GUI screen/overlay creation

Review these files for format, depth, and style benchmarks.
